...
def test_wsl_command_uses_windows_terminal_bridge(monkeypatch, tmp_path):
    monkeypatch.setattr(claude_terminal.platform, "system", lambda: "Linux")
    monkeypatch.setattr(
        claude_terminal.shutil,
        "which",
        lambda name: {"wt.exe": "/mnt/c/wt.exe", "powershell.exe": "/mnt/c/powershell.exe"}.get(name),
    )
    ...
    assert host == "wsl"
    assert command[:6] == ["/mnt/c/wt.exe", "-w", "0", "new-tab", "--title", "session"]
    assert "UColt" in command[-1]
    assert str(tmp_path / "launch.sh") in command[-1]
