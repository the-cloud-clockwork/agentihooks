...
def reconcile_live_sessions() -> dict:
        ...
        try:
            ...
            for i, p in enumerate(parts):
                if p == "--resume" and i + 1 < len(parts):
                    candidate = parts[i + 1]
                    # Basic UUID shape check (36 chars, 4 dashes)
    ...
    for cwd, pids in pids_by_cwd.items():
        ...
        # Preferred matching: each PID with --resume <sid> maps directly to
        # that session. Falls back to JSONL mtime pairing only for PIDs
        # without --resume (fresh session_id generated by Claude Code at
        # launch, not yet bound to an existing JSONL).
        ...
        for pid in pids_sorted:
            ...
            if not sid:
                continue
            matched_by_cmdline.add(pid)
            entry = sessions.get(sid)
            if entry is not None and entry.get("status") in ("closed", "superseded"):
                # Operator explicitly closed this — don't resurrect it.
                summary["unmatched_pids"] += 1
                continue
            if entry is None:
                entry = {
                    "started_at": now_iso,
        ...
        # Fallback mtime pairing for PIDs without --resume
        unresumed_pids = [p for p in pids_sorted if p not in matched_by_cmdline]
        # Skip JSONLs that are already owned by an alive session
        claimed_sids = {
            sid for sid, info in sessions.items() if info.get("status") == "alive" and info.get("cwd") == cwd
        }
        free_jsonls = [(m, j) for m, j in jsonls if j.stem not in claimed_sids]

        for pid, (_mtime, jsonl) in zip(unresumed_pids, free_jsonls):
            sid = jsonl.stem
            entry = sessions.get(sid)
            if entry is not None and entry.get("status") in ("closed", "superseded"):
                summary["unmatched_pids"] += 1
