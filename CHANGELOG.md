...
- The packaged AgentiHooks toolbelt rule is action-first and makes handling
  injected enforcement, broadcast, and hook-block context mandatory.
- The quota-usage banner states its configured tool-call display cadence.
...
- Claude Code sessions receive an `ATTENTION TO QOUTA USAGE` hook banner with
  native 5-hour and 7-day remaining quota on every user prompt and every fifth
  tool call.
