...
The state directory location is overridable via the `AGENTIHOOKS_HOME` env var. Set it before any `agentihooks` command and the entire stack — `install.py`, every hook, the brain outbox, the MCP cache, session flags — resolves through that path.
...
# 5. Install everything (hooks, skills, agents, MCPs, bashrc)
agentihooks init
...
| `active-sessions.json` | Session registry — naturally repopulated |
...
| `state/` | Transient state files — re-derived |
| `*.pid`, `ctx_refresh_*.json` | Stale PIDs + session refresh signals |
| `~/.claude/{rules,skills,agents,commands}/` symlinks | Re-symlinked by init |
| `~/.claude/{settings.json,CLAUDE.md}` | Regenerated by init |
